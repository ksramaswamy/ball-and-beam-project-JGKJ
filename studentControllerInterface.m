classdef studentControllerInterface < matlab.System
    properties (Access = private)
        ...
        xm = [-.19 0 -50*pi/180 0 0]';
        n = 5;
        Evv = diag([.04 .01 .03 .01 0.01]).^2;
        Eww = diag([.001 .001]).^2;
...
%             obj.dtheta = obj.xm(4);
            ...
            switch(obj.controllerType)
                case 1
                    ...
                    V_servoVirtual = k_servo * (obj.theta_d - theta) - kd_servo*obj.dtheta;
                case 2
                    %% Sontag Controller
                    ...
                    V_servoVirtual = ...
-(1.0*((0.418*sin(x3) + x4^2*cos(x3)^2*(0.00255*x1 - 5.42e-4))*(1633.0*x1 + 1799.0*x2 + 158.0*x3 + 3.52*x4) + x2*(2577.0*x1 + 1633.0*x2 + 125.0*x3 + 2.74*x4) + x4*(125.0*x1 + 158.0*x2 + 41.3*x3 + 0.934*x4) - 40.0*x4*(2.74*x1 + 3.52*x2 + 0.934*x3 + 0.0218*x4) + ((110.0*x1 + 141.0*x2 + 37.4*x3 + 0.871*x4)^4 + ((0.418*sin(x3) + x4^2*cos(x3)^2*(0.00255*x1 - 5.42e-4))*(1633.0*x1 + 1799.0*x2 + 158.0*x3 + 3.52*x4) + x2*(2577.0*x1 + 1633.0*x2 + 125.0*x3 + 2.74*x4) + x4*(125.0*x1 + 158.0*x2 + 41.3*x3 + 0.934*x4) - 40.0*x4*(2.74*x1 + 3.52*x2 + 0.934*x3 + 0.0218*x4))^2)^(1/2)))/(110.0*x1 + 141.0*x2 + 37.4*x3 + 0.871*x4);

                    % Saturate output to conserve energy
                    saturate = 1;
                    V_servoVirtual = max(min(V_servoVirtual,saturate),-saturate);
                case 3
                    %% LQR Controller
                    K = [31.6228   37.6022    9.3513    0.2114];
                    x1 = p_ball - (p_ball_ref);
                    x2 = obj.v_ball - (v_ball_ref);
                    x3 = theta;
                    x4 = obj.dtheta;

                    V_servoVirtual = -K*[x1;x2;x3;x4];
                    % Saturate output to conserve energy
            ...
            if V_servoVirtual >= 0 && V_servoVirtual <= 5
                V_servo = V_servoVirtual + obj.GravComp + obj.FrictionComp;
            elseif V_servoVirtual < 0
                V_servo = V_servoVirtual + obj.GravComp - obj.FrictionComp;
            else
                V_servo = 5;
            end
